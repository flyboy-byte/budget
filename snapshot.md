...
_Auto-generated by `scripts/make_snapshot.py` from README.md, DEVELOPMENT.md, CLAUDE.md, ARCHITECTURE.md, IDEAS.md, IMPLEMENTATION_HISTORY.md as of 2026-09-03 — for pasting into another chat/tool for review. MAKING_PUBLIC.md and security-audit.md are deliberately excluded (the former is about this repo's own visibility, not the app itself; the latter is written for a Claude instance running on the VPS, not for pasting elsewhere). Regenerate rather than hand-edit — this file is a derived view, not a source of truth; the individual .md files are._
